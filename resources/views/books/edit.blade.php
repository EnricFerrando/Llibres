    ...
    <div class="card">
        ...
        <form action="{{ route('books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" required value="{{ old('title', optional($book)->title) }}" class="form-input mt-1">
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @if(optional($book)->image)
                        <img id="coverPreview" class="mt-4 w-48 h-auto rounded-xl shadow" src="{{ asset('storage/'.optional($book)->image) }}" data-default-src="{{ asset('storage/'.optional($book)->image) }}" alt="Previsualització de la portada">
                    @else
